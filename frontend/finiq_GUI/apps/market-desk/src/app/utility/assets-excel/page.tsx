...
export default function AssetExcelUtilityPage() {
  ...
  const [sheetPreviewCache, setSheetPreviewCache] = useState<Record<string, SheetPayload>>({});
  ...
  useEffect(() => {
    ...
    fetch(`/api/assets/excels/output?${params.toString()}`)
      .then((response) => {
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        return response.json();
      })
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const params = new URLSearchParams({ row_limit: "20" });
    if (selectedSheet) params.set("sheet_name", selectedSheet);
    if (sheetPreviewMode === "interpreted") params.set("interpreted", "true");
    const cacheKey = JSON.stringify({ selectedPreviewFile, selectedSheet, sheetPreviewMode });
    const cached = sheetPreviewCache[cacheKey];
    if (cached) {
      setSheetPayload(cached);
      if (!selectedSheet) setSelectedSheet(cached.sheet_name || cached.sheet_names?.[0] || "");
      return;
    }
    fetch(`/api/assets/excels/${encodePath(selectedPreviewFile)}?${params.toString()}`)
      .then((response) => {
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        return response.json();
      })
      .then((data) => {
        ...
        setSheetPreviewCache((current) => ({ ...current, [cacheKey]: data }));
  ...
  }, [selectedPreviewFile, selectedSheet, sheetPreviewMode, sheetPreviewCache]);
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await fetch("/api/assets/excels/preview-conversion", {
        method: "POST",
  ...
  const handleStart = async () => {
    ...
    try {
      const response = await fetch("/api/assets/excels/convert-wide-parquet/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          output_directory: outputDirectory,
        ...
        <section className="space-y-6">
          <Card className="sticky top-6 dark:bg-[#161b22] dark:border-[#30363d]">
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={handleStart} disabled={!!activeJobId || loading || !selectedFiles.length || !previewIsCurrent} className="w-full">
