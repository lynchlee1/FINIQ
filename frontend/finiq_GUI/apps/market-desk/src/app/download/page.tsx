...
export default function DownloadPage() {
  ...
  const handlePickPath = async () => {
    try {
      ...
      const data = await response.json();
      if (data.path) setOutputDirectory(data.path);
    } catch (err: any) {
      setStatus(err.message);
      setIsErrorStatus(true);
    }
        ...
        <section className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="icon" onClick={handlePickPath}>
          ...
          <Card>
            ...
            <CardContent className="grid md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <Label>종료 페이지</Label>
                <Input type="number" placeholder="전체" value={endPage} onChange={(e) => setEndPage(e.target.value)} />
              </div>
              <div className="flex items-center space-x-2 pt-8">
                <Checkbox id="lastReportOnly" checked={lastReportOnly} onCheckedChange={(v) => setLastReportOnly(!!v)} />
                <Label htmlFor="lastReportOnly" className="cursor-pointer">최종보고서만</Label>
              </div>
              <div className="flex items-center space-x-2 pt-8">
                <Checkbox id="resumeYearly" checked={resumeYearly} onCheckedChange={(v) => setResumeYearly(!!v)} />
                <Label htmlFor="resumeYearly" className="cursor-pointer">연간 작업 재개</Label>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>공시 종류</CardTitle>
              <CardDescription>다운로드할 공시 종류를 선택하세요.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {options?.disclosure_groups.map((group) => (
                <div key={group.suffix} className="border rounded-lg overflow-hidden">
                  <div className="bg-slate-50 px-4 py-2 border-b flex items-center justify-between">
                    <span className="font-semibold text-sm">{group.label} ({group.items.length})</span>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => selectGroup(group.suffix, group.items)}>전체 선택</Button>
                      <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => clearGroup(group.suffix)}>전체 해제</Button>
                    </div>
                  </div>
          ...
          <Card className="sticky top-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>작업 상태</Label>
                <div className={cn(
                  "p-3 rounded-lg border text-sm font-medium min-h-[100px] whitespace-pre-wrap font-mono text-xs",
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 rounded-lg border bg-slate-900 text-slate-50 font-mono text-[10px] overflow-auto max-h-[400px]">
