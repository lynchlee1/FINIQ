...
export default function IntegratedMergePage() {
  ...
  const handlePickPath = async (target: "input" | "output") => {
    try {
      ...
      const data = await response.json();
      if (data.path) {
        if (target === "input") setInputDir(data.path);
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Dataset Merge</p>
              <CardTitle className="text-xl">Parquet 병합</CardTitle>
            </div>
            <Button 
          ...
          <CardContent className="pt-6 space-y-6">
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="inputDir">입력 데이터셋 폴더</Label>
                <div className="flex gap-2">
                  ...
                  />
                  <Button variant="outline" size="icon" onClick={() => handlePickPath("input")}>
                    <FolderOpen className="h-4 w-4" />
                  </Button>
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="outputDir">병합 결과 폴더</Label>
                <div className="flex gap-2">
                  <Input 
                    id="outputDir"
                    value={outputDir}
