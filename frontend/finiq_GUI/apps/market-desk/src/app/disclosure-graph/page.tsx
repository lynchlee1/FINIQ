...
type DisclosureGraphDocument = GraphData & {
  format: "finiq_disclosure_graph_v2";
  metadata: {
    built_at?: string;
    total_nodes?: number;
    total_edges?: number;
...
const SAMPLE_WORKSPACE_PATH = "examples/disclosure-graph-workspace";
...
export default function DisclosureGraphPage() {
  ...
  const [status, setStatus] = useState("저장된 그래프를 불러오거나 새로 생성하세요.");
    ...
    >
      <div className="space-y-6">
        ...
        >
          ...
          <p className="mt-3 text-xs text-[var(--tv-muted)]">
            프로젝트 예시 작업공간: <code>{SAMPLE_WORKSPACE_PATH}</code> · 폴더를 선택한 뒤 저장 결과를 불러오세요.
          </p>
          <div
