...
컴포넌트 추가 계약은 [컴포넌트 디자인](./components/index.md)에 둔다. 기타 시각 규칙과 UI 용어는 이 문서를 따른다.
...
| Disclosure automation manual discovery/run action | 동기화 | Uses each detail page's execution path in task order. Before 01 runs, compare the saved and current KIND counts for each requested range. |
| Disclosure automation KIND count conflict | 건수 충돌 | Use in `알림` when a saved range and the current KIND result report different disclosure counts. |
| Disclosure automation KIND count conflict confirmation | 다시 받기 | Right-side `알림` action that permits the requested range to be downloaded again through the regular 01 execution path. |
...
| KIND source HTML unavailable | KIND 원본 없음 | Use only after an internal HTML download fails and a direct revalidation confirms that KIND returns no content path. Invalid HTML remains a failed download. Save a canonical placeholder in the receipt's normal HTML slot, hash it in the manifest, retain the reason in the manifest and job log, and show its count in inspection results. The placeholder counts as the physically present requested file while remaining explicitly unavailable through section splitting and parsing. |
