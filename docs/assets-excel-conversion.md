...
- `Quantiwise - 병합하기`는 사용자가 고른 `병합 대상 데이터 경로` 안에서 같은 계정 Parquet 파일을 2개씩 하나 이상 선택해 `병합 결과 데이터 경로`에 결과를 저장한다.
...
- 병합하기는 `병합 대상 데이터 경로` 하나를 입력으로 사용하고, 그 안에서 선택한 같은 계정 Parquet 파일 2개 묶음들의 날짜/종목코드 값을 `병합 결과 데이터 경로`에 병합한다.
- `Quantiwise - 병합하기`의 `병합대상 모아보기`는 같은 계정 Parquet 파일이 2개 이상 있는 항목만 표시한다.
- `동일 폴더에서 작업하기`가 활성화되면 `병합 결과 데이터 경로` 값과 관계없이 `병합 대상 데이터 경로`에 병합 결과를 저장한다. 기본값은 비활성화다.
- `병합된 요소 정리하기`가 활성화되면 병합 성공 후에만 선택된 원본 Parquet 파일을 `병합 대상 데이터 경로/merged`로 옮긴다. 기본값은 활성화다.
- `병합 대상 데이터 경로/merged`에 같은 이름의 보관 파일이 이미 있으면 기존 파일을 덮어쓰지 않고 `__2`, `__3` suffix로 비어 있는 보관 파일명을 사용한다.
- `Quantiwise - 병합하기`는 병합 결과와 Code-Name mapping을 `병합 결과 데이터 경로` 아래 임시 폴더에 먼저 저장하고, 모든 병합이 성공한 뒤 최종 파일로 승격한다. 병합 중 취소/실패하면 최종 Parquet를 남기지 않는다.
- `중복 검사하기`는 기본적으로 `병합 대상 데이터 경로` 바로 안의 Parquet만 검사한다. `내부까지 검사`를 켠 경우에만 모든 하위 폴더까지 포함한다. 같은 계정 Parquet끼리 date, 종목코드, 내부 값을 비교하고, 한 파일의 모든 `(date, 종목코드, 값)`이 더 완전한 같은 계정 파일에 손실 없이 포함되면 더 작은 파일을 삭제 후보로 표시한다. 삭제는 사용자가 `확인했습니다.`를 입력하고 삭제 허가를 체크한 뒤에만 실행한다.
...
- 병합 실행도 결과 Parquet와 Code-Name mapping을 먼저 임시 폴더에 저장하고, 성공 후 최종 경로로 이동한다. 저장 중 작업이 중단되면 임시 폴더를 정리하고 최종 Parquet를 남기지 않는다.
...
- Quantiwise 경로 입력은 모두 명시 지정한다. 빈 원본 데이터 경로, 데이터 경로, 병합 대상 데이터 경로, 병합 결과 데이터 경로를 기본값으로 대체하지 않는다.
- 병합하기는 `병합 대상 데이터 경로` 안의 account Parquet 파일을 같은 계정별로 정확히 2개 선택한 경우만 실행한다. 경로 안의 모든 파일을 한 번에 병합하지 않는다.
