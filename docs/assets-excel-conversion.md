...
- `Quantiwise - 병합하기`는 병합 결과와 Code-Name mapping을 `병합 결과 경로` 아래 임시 폴더에 먼저 저장하고, 모든 병합이 성공한 뒤 최종 파일로 승격한다. 병합 중 취소/실패하면 최종 Parquet를 남기지 않는다.
- `중복 검사하기`는 기본적으로 `병합 대상 경로` 바로 안의 Parquet만 검사한다. `내부까지 검사`를 켠 경우에만 모든 하위 폴더까지 포함한다. 같은 계정 Parquet끼리 date, 종목코드, 내부 값을 비교하고, 한 파일의 모든 `(date, 종목코드, 값)`이 더 완전한 같은 계정 파일에 손실 없이 포함되면 더 작은 파일을 삭제 후보로 표시한다. 삭제는 사용자가 `확인했습니다.`를 입력하고 삭제 허가를 체크한 뒤에만 실행한다.
