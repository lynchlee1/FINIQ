...
- 병합하기는 기존 Parquet 경로와 병합할 Parquet 경로를 별도로 받아 같은 계정/날짜/종목코드 값을 병합한다.
- 변환 전 확인 API는 저장 없이 Sheet/계정 매핑, 기존 출력 감지, 충돌 샘플, skipped Sheet를 반환한다.
- `Quantiwise 변환`은 실행 시 변환 전 확인을 자동 수행한다. 별도 확인 버튼을 두지 않는다.
...
- 변환 전 확인 UI는 정상 매핑, 미매핑, 형식 오류, 충돌을 분리해서 보여준다.
...
- 변환 실행 시 Sheet/계정 매핑과 skipped Sheet를 자동 확인할 수 있어야 한다.
