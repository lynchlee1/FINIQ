...
- `Quantiwise - 변환하기`는 실행 전 `계정-ID 매핑`을 편집, 추가, 삭제할 수 있고, 변환/미리보기 스캔은 요청 payload의 매핑 목록을 기본 매핑 대신 사용한다.
...
- `Quantiwise - 변환하기`는 변환 완료 후 생성된 Sheet Parquet를 다시 읽어 `Quantiwise - 미리보기`의 `Sheet 읽기/미리보기`와 같은 카드/선택/표 양식으로 보여준다.
...
- Quantiwise 경로 입력은 모두 명시 지정한다. 빈 원본 데이터 경로, 기존 Parquet 경로, 병합할 Parquet 경로, 데이터 경로를 기본값으로 대체하지 않는다.
...
- 기존 출력 폴더의 Parquet/manifest 존재 여부는 내부 선택지 구성에만 사용하고, `Quantiwise - 변환하기` 실행 화면에서 기존 결과 감지 경고를 표시하지 않는다.
