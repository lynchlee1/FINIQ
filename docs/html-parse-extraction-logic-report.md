...
`_main_rights_rows()`는 정정표가 아닌 table 중 아래 3개 조건을 모두 만족하는 table을 찾는다.
...
이 table을 찾지 못하면 본문 누락 viewer일 수 있다고 보고 별도 본문 HTML을 찾아 다시 `build_base_record()`를 수행한다.

의존 가정:

- 유무상증자 결정 공시는 위 3개 라벨을 포함한다.
- viewer HTML에 본문이 없으면 같은 파일명으로 별도 본문 HTML이 저장되어 있을 수 있다.

틀릴 수 있는 경우:

- 라벨 표현이 다른 경우
- 본문 HTML 저장 위치나 파일명이 예상과 다른 경우
- 유사 라벨을 가진 다른 table이 먼저 매칭되는 경우
