...
> **문서 목적:** 회사별 공시 시간선과 관계 그래프를 조회·편집·내보낼 때 지키는 규칙과 오류 처리 방식을 설명한다.
>
> **다루지 않는 내용:** 03·07단계 결과를 표준 graph JSON으로 만들어 저장하는 기능은 [공시 관계 그래프](../../disclosures/09-disclosure-graph/reference.md), 주가-공시 차트는 [Chart View](../chart-view/reference.md), Triple Barrier는 [공시 분석](../disclosure-analysis/reference.md)에서 설명한다.
...
- Neo4j로 동기화할 때 node에서 `riskLevel`이나 `riskDescription`이 `null`이면 위험도나 설명을 임의로 만들지 않고 `null`을 전달한다.
