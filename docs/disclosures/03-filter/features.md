...
- 새 기간이 기존 기간 뒤에 추가되고 02단계가 기존 행 순서를 유지한다는 운영 전제를 사용한다. 과거 기간을 끼워 넣거나 기존 행 순서를 바꾸는 갱신은 증분 실행 대상이 아니다.
...
- 조건 블록 연결자는 `AND`, `XOR`, `OR`를 지원하며 `XOR`는 양쪽 조건 중 정확히 하나만 참일 때 참이다. 우선순위는 `AND`, `XOR`, `OR` 순이며 괄호를 먼저 계산한다.
- `between` 연산자에 두 값을 지정하지 않으면 실패 처리한다.
- 공시에 비어 있지 않은 텍스트 `acpt_no`가 없으면 실패 처리한다. `acpt_no`는 로마자를 포함할 수 있으며 숫자로 변환하지 않는다.
...
검색 조건에 이름을 붙여 저장하고 조건, 실행 상태, 완료 결과 또는 중단 결과를 단일 JSON에서 관리한다. 실행 중에는 원본을 바꾸지 않고 같은 폴더의 숨김 임시 JSON에서 상태와 증분 결과를 만든다.

#### Defaults and Exceptions

- 새 preset의 초기 상태는 `ready`이며 변환 유형과 조건을 함께 기록한다.
- 기존 preset과 변환 유형·조건이 같으면 완료·중단 결과를 유지하고, 둘 중 하나가 달라지면 상태와 결과를 대기로 초기화한다.
- 완료 목록은 `result`, 중단된 증분 목록은 `pending.result`에 저장한다.
- 모든 건수 검증을 통과한 완료 결과만 원본 JSON으로 원자적으로 교체한다.
- 이전 형식의 결과 파일은 preset 목록에서 제외하며 자동으로 바꾸지 않는다.
- 손상됐거나 필요한 상태가 없는 preset을 임의로 고치거나 결과 파일로 대신하지 않고 실패 처리한다.
- 파일명에 경로 구분자를 사용하거나 저장된 조건과 실행 요청 조건이 다르면 실패 처리한다.
...
- 실행 중인 workflow는 동시에 다시 실행하거나 저장·이름변경·삭제할 수 없다.
...
공시내역 필터링, 공시원문 변환과 공시 자동화에서 같은 조건검색 preset 목록을 제공하고, 선택한 preset의 변환 유형과 조건을 실행 입력으로 사용한다.

#### Defaults and Exceptions

- 선택 목록은 `03-filter` 바로 아래의 조건검색 preset만 읽는다.
- 공시내역 필터링 화면에는 수동 `불러오기` 버튼을 표시하지 않는다.
- 03단계와 후속 단계의 실행 설정에는 선택한 preset 이름이 들어간다.
