...
- 문서 전체의 허용 fallback은 두 개뿐이다. 첫째는 구조화 안건 schema 자체가 없을 때의 direct-cell legacy 안건 선택이고, 둘째는 이름으로 선임 안건을 연결하지 못했을 때 같은 직책의 선임 세부내역 한 건과 이름 없는 안건 한 건이 각각 정확히 하나인 경우의 연결이다. 후보자·선택권 대상자 문법은 canonical `candidate` field와 제목의 명시 문법을 독립적으로 수집·중복 제거하며, 다른 field나 DOM source로 대체하지 않는다.
...
- 성명·후보자·주식매수선택권 부여 대상자와 이름이 명시된 개인 주주제안자는 사람, 다른 법인명과 회사로 표시된 주주제안자는 기관으로 분류한다. 이름 길이나 회사명 접미사만으로 사람과 기관을 서로 바꾸지 않는다.
...
- 직접 자식 셀이 정확히 `정정항목`, `정정전`, `정정후`인 비교 표에서 참고사항 행의 `정정후`가 후보 명단과 `주주제안(제안자|제안인: NAME)에 의한 후보자`를 완전한 문장으로 명시하면, 같은 공시의 선임 세부내역과 직책별 안건을 대조해 그 제안자를 해당 안건에 연결한다. 이 typed source는 안건 선택이나 일반 참고사항 추출의 fallback으로 사용하지 않는다.
...
- 관계 유형은 회의-안건 `includes`, 후보자-회사 `candidate_for`, 선임된 사람-회사 `elected_as`, 해임·사임한 사람-회사 `removed_from`·`resigned_from`, 안건 대상 `subject_of`, 제안자-안건 `proposed`, 사람-다른 기관 `serves_at`, 선택권 대상자-회사 `option_granted_by`, 회계법인-회사 `external_auditor_of`, 전자투표 수탁·관리기관-회의 `electronic_voting_manager_for`, 명명된 전자투표 시스템 제공기관-회의 `electronic_voting_system_provider_for`를 사용한다.
...
- NOTICE의 후보자는 `candidate_for`로만 표현한다. RESULT에서 명시적으로 가결된 선임과 주식매수선택권 부여만 각각 `elected_as`, `option_granted_by`로 표현한다.
...
- 첫 번째 정확한 `기타 투자판단에 참고할 사항` 제목 바로 다음 값 행 하나를 source로 고정해 이름과 행위가 함께 명시된 외부감사인과 전자투표 관리기관을 추출한다. 선택한 값이 비거나 해석되지 않아도 뒤의 같은 제목으로 바꾸지 않는다. 이전 감사인과 현재 감사인은 `external_auditor_of.attributes.state`의 `former`, `current`로 구분한다. 전자투표의 직접 위탁·위임 또는 `관리기관: NAME` 선언은 `electronic_voting_manager_for`로 만들고 행위, 예정·현재 상태, 명시된 서비스 목록을 보존한다.
- 같은 값 행에 이번 총회의 긍정적인 전자투표 사용 선언, 실제 행사기간, 알려진 기관명이 붙은 시스템 또는 서비스 표기, 그 기관의 URL이 함께 있으면 `electronic_voting_system_provider_for`로 만든다. 전자위임장 시스템도 함께 명시된 경우에만 해당 서비스를 관계 속성에 추가한다. 같은 기관에 직접 관리 위탁 관계가 있으면 제공자 관계를 중복 생성하지 않는다.
...
- 정정 비교 표에서는 `정정전`, 다른 정정항목, 비표준 header, 임의 안내문을 제안자 근거로 사용하지 않는다. 이름이 선임 세부내역에 없거나 같은 직책 안건이 하나로 확정되지 않으면 관계를 만들지 않는다.
...
- 이름 없는 `외부감사인 선임보고`, 일반 전자투표 안내, URL·시스템명·이용약관 중 일부만 있는 설명에서는 기관을 추정하지 않는다. 전자투표 미사용·중단 문구도 현재 제공 관계로 만들지 않는다. 기관명의 오탈자와 약칭은 07단계에서 임의 교정하지 않는다.
...
- 자유문 안건의 후보자, 제안자, 선택권 대상자 추출은 알려진 문장 구조에 한정되며 모든 표현을 완전하게 해석한다고 보장하지 않는다.
