...
주주총회 NOTICE와 RESULT HTML에서 안건, 사람, 기관과 관계를 결정론적으로 추출한다. 후보자를 현직 임원으로 오인하거나, 이름이 없는 주체를 추정하거나, 파싱 실패를 다른 field·DOM·metadata로 보완하지 않는 것을 우선했다.

AI는 실제 원문 golden 기대값을 판정하고 반례를 검토하는 데만 사용했다. production runtime에는 LLM 호출이나 모델 의존성이 없다.
...
현재 semantic contract는 19개 관계 유형을 사용한다. 세부 목록과 endpoint 계약은 상세 handoff와 같은 디렉터리의 `features.md`, `reference.md`에 기록했다.
...
Golden은 단순 hash 승인 장치가 아니다. 출력이 달라지면 원문과 semantic diff를 먼저 검토하며, 의미 검토 없는 manifest hash 갱신은 금지한다.

### fallback 축소

실패-triggered fallback은 다음 두 개만 유지했다.
...
주식 양도, 외부감사인, 사업목적 합병, 전자투표 위탁과 행사기간의 여러 표현은 순차 rescue가 아니라 같은 canonical source에 대한 독립 후보 수집·중복 제거로 바꿨다.

### entity identity 사전 결정

생년월 없는 사람을 나중에 동일 이름의 born entity로 업그레이드하는 순서 의존 병합을 제거했다. 대신 문서 전체 election row를 먼저 읽고 exact person key별 nonempty 생년월 집합을 만든다.
...
이 규칙은 appointment, 일반 `agenda_candidate`, 생년월이 빈 election row에 동일하게 적용한다. registry 사후 lookup이나 upgrade가 아니므로 입력 순서에 따라 인물이 합쳐지지 않는다.

### high-precision 경계

- `elected_as`, `removed_from`, `resigned_from`, `option_granted_by`는 RESULT의 명시적 passed에만 생성
- 관계 evidence에 비어 있지 않은 `raw_text` 필수
- major career는 현재 표식+기관+지원 직책의 완전한 한 줄만 허용
- 같은 셀의 기관명 줄+직책 줄은 완전한 multiline 문법으로만 허용
- `사내\n이사`, `부\n회장`처럼 직책 단어 내부가 갈린 표면은 제외
...
| 시도 | 결과 | 교훈 |
| --- | --- | --- |
| grouped header면 항상 둘째 줄 `안건` 열 사용 | 실제 golden 4건에서 안건 34개와 관계 소실 | 둘째 줄에 명시적 `안건`이 있을 때만 사용하고, 아니면 첫 줄 `회의목적사항`을 유지한다. |
...
| 변경 중인 worktree에서 전수 감사 | start/end/live hash drift로 결과 무효화 | immutable snapshot에서 실행하고 세 hash가 모두 같을 때만 결과를 채택한다. |

구체 receipt, 반례 문장, 제거한 fallback 목록은 상세 handoff의 “이번 시도에서 드러난 실패와 교훈” 절에 보존했다.

## 5. 검증 결과

최종 수치는 현재 production hash가 고정된 뒤 이 절과 `PLANS.md`에 함께 기록한다.
...
전체 repository 실패 4건은 이번 주주총회 범위 밖이다.

- classification SQLite contract 2건
- HTML download `max_workers` validation order 2건

최종 immutable 2026 audit은 4,389 external/internal 쌍을 모두 파싱했고 exception과 hard invariant 위반은 0건이었다.
...
Snapshot start/end와 live-end의 production hash가 모두 일치했다.

## 6. 현재 한계

- 최신 production으로 2008–2026년 71,965쌍 전체 이력 감사를 다시 완료하지 않았다.
...
- AI adjudication 기록에는 reviewer 역할과 판단은 있지만, hosted model의 정확한 snapshot ID와 raw invocation transcript는 저장소에서 독립 재현할 수 없다.
...
기능 확장은 그 이후 별도 변경으로 다룬다. 우선순위는 NOTICE/RESULT meeting identity 설계, 모호하지 않은 자유문 recall 표본 조사, issuer 범위를 넘는 person identity 정책이다.

## 8. 마감 판정

현재 코드는 주주총회 범위의 focused/golden 계약과 최신 2026 immutable audit을 통과했고, 실패 접근과 재개 절차가 문서화되어 있다. 이 세션에서는 기능을 더 넓히지 않는다. 전체 71,965쌍 이력 검증은 다음 세션의 첫 작업으로 남긴다.
