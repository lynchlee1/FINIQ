...
**`rights_issuance` · 유무상증자파싱** — 상세 필드를 지원한다.
...
- 접수번호 예시는 일부만 보여 주고 전체 개수는 모두 계산한다.
...
4. 현재 mode parser가 그 위치를 어떻게 찾는지 추출 규칙과 비교한다.

#### 수정하고 검증하기

1. 해당 mode parser의 추출 함수와 보조 함수 책임을 확인하고 추출 규칙을 고친다.
