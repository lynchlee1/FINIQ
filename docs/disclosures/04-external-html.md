...
- 자동화 04단계는 새 HTML 디렉터리와 새 압축 JSON을 모두 검증한 뒤 함께 게시한다. 어느 한쪽 게시가 실패하면 둘 다 이전 세대로 복원한다.
- 두 결과가 모두 게시된 뒤 이전 백업이나 임시 디렉터리 삭제만 실패하면 새 세대를 유지하고 정리 경고를 반환한다.
