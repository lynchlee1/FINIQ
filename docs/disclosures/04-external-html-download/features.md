...
- **중요 — 사용자가 직접 명시한 설계 의도(2026-08-29):** 04단계의 HTML 판별은 과거 KIND HTML과의 레거시 호환을 위해 의도적으로 넓게 유지한다. `<html`이 있는 일반 문서뿐 아니라 `openDisclsViewer` 식별자가 있는 과거 외부 화면과, wrapper 없이 `<P>`로 시작해 `<TABLE>`을 포함하는 과거 본문 조각도 정상 후보로 인정한다. 따라서 완전한 `<html>/<head>/<body>` wrapper가 없다는 이유만으로 무조건 오류로 판정하면 안 된다. 이 호환 범위를 더 엄격하게 바꾸는 작업은 사용자가 차후 별도로 다루기로 직접 지정했으며, 현재 구현 오류 수정 범위가 아니다.
