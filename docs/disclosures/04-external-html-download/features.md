...
- `외부 HTML 저장`에서는 `기존 원문 데이터 검사`와 `미저장 원문 다운로드`를 별도 행으로 나누지 않는다. 다운로드 필요 수는 파생 모드를 중복 합산하지 않고 원문 폴더를 실제 소유하는 기본 모드만 합산한다. 미저장·해시 불일치가 있으면 기존 검사 행의 `검사하기`가 `재다운로드`로 바뀌며, 누락·해시 불일치가 있는 기본 모드만 순차 처리한 뒤 모든 모드를 다시 검사한다.
