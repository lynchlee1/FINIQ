...
- `외부 HTML 압축`에서도 `기존 데이터 검토`를 첫 카드로 표시한다. 저장 세부 페이지는 기존 원문 재사용과 미저장분 다운로드를 검사하고, 압축 세부 페이지는 저장된 `compressed-external-html.json`을 검사한다.
- 압축 파일 검사는 JSON 형식, 현재 필터 대상의 누락·추가·중복과 각 record의 원문 hash·size를 현재 압축 로직으로 다시 계산한 결과와 대조한다. 파일이 없거나 내용이 다르면 `사용 불가`로 표시한다.
