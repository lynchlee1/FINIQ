...
- `외부 HTML 압축`에서도 `기존 데이터 검토`를 첫 카드로 표시한다. 저장 세부 페이지는 기존 원문 재사용과 미저장분 다운로드를 검사하고, 압축 세부 페이지는 선택 필터와 무관하게 모든 기본·파생 모드의 `compressed-external-html.json`을 검사한다.
- 압축 파일 검사는 모드별 JSON 형식, 대상의 누락·추가·중복과 각 record의 원문 hash·size를 현재 압축 로직으로 다시 계산한 결과와 대조해 모두 출력한다. 파일이 없거나 내용이 다르면 `사용 불가`로 표시한다.
- 하나라도 불일치하면 `전부 재생성`으로 모든 기본 모드의 압축 파일을 다시 만들며, 파생 모드는 상위 모드가 소유한 새 파일을 공유한다.
- 세부 페이지를 바꾸면 이전 세부 페이지의 검사 결과를 지우고 새 검사 전 상태로 돌아간다.
- 기존 원문 무결성과 미저장 원문 수는 같은 검사 요청에서 계산한다. `검사하기`가 있는 첫 행에는 순번을 표시한다. 미저장 원문 행은 순번을 비우고, 첫 행의 검사 중에도 회전 아이콘 없이 `대기`를 유지한 뒤 요청이 끝나면 결과를 표시한다.
