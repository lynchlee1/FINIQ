...
_INSTITUTION_CHARS = r"A-Za-z0-9가-힣&·ㆍ.㈜"
_VOTING_MANAGER_BODY = (
    rf"(?=[{_INSTITUTION_CHARS}]{{4,40}}\s*(?:에|에게)\s*(?:위탁|위임))"
    rf"[{_INSTITUTION_CHARS}]*[가-힣][{_INSTITUTION_CHARS}]*|"
    r"[A-Z][A-Z0-9&.]+(?:\s+[A-Z][A-Z0-9&.]+){1,3}"
)
_VOTING_DELEGATION_RE = re.compile(
    rf"(?<![{_INSTITUTION_CHARS}])(?P<name>{_VOTING_MANAGER_BODY})"
    rf"\s*(?:에|에게)\s*(?P<action>위탁|위임)"
)
_VOTING_MANAGER_DECLARATION_RE = re.compile(
    rf"전자\s*투표\s*관리\s*기관\s*[:：]\s*"
    rf"(?P<name>[{_INSTITUTION_CHARS}]{{2,40}})\s*$"
)
_ELECTRONIC_VOTING_RE = re.compile(r"전자\s*투표")
_ELECTRONIC_PROXY_RE = re.compile(r"전자\s*위임장")
_PLANNED_DELEGATION_RE = re.compile(
    r"^\s*(?:(?:할\s*)?(?:예정|계획)|하기로)"
)
_VOTING_PROVIDER_URLS = {
    "삼성증권": "samsungpop.com",
    "한국예탁결제원": "ksd.or.kr",
}
_VOTING_PROVIDER_NAME_PATTERN = "|".join(
    re.escape(name) for name in _VOTING_PROVIDER_URLS
)
_VOTING_SYSTEM_PROVIDER_RE = re.compile(
    rf"(?:(?P<named>{_VOTING_PROVIDER_NAME_PATTERN})\s*전자\s*투표\s*시스템|"
    rf"전자\s*투표\s*[·ㆍ]?\s*전자\s*위임장\s*권유\s*시스템\s*"
    rf"\(\s*(?P<parenthesized>{_VOTING_PROVIDER_NAME_PATTERN})\s*\)|"
    rf"(?P<terms>{_VOTING_PROVIDER_NAME_PATTERN})\s*전자\s*투표\s*서비스\s*이용\s*약관)"
)
_VOTING_AFFIRMATIVE_USE_RE = re.compile(
    r"(?:활용\s*(?:하기로|할|합니다|하였|했|예정)|"
    r"도입\s*(?:하였|했|합니다|하기로|예정)|시행\s*중|"
    r"전자적\s*방법으로.{0,80}?의결권을?\s*행사|"
    r"의결권\s*행사.{0,80}?(?:가능|할\s*수\s*있)|"
    r"의결권을?\s*행사.{0,40}?(?:가능|할\s*수\s*있))"
)
_VOTING_NEGATIVE_USE_RE = re.compile(
    r"(?:활용|도입|시행).{0,16}?"
    r"(?:하지\s*(?:않|아니)|할\s*수\s*없|중단|철회|취소)|"
    r"의결권.{0,40}?행사(?:하지\s*(?:않|아니)|할\s*수\s*없|불가)"
)
_VOTING_PERIOD_LABEL_RE = re.compile(
    r"전자\s*투표\s*(?:(?:행사\s*)?기간|행사\s*[:：])"
)
_DATE_TOKEN_RE = re.compile(
    r"(?:(?:19|20)\d{2}\s*[년./-]\s*)?"
    r"\d{1,2}\s*[월./-]\s*\d{1,2}\s*일?"
)
_RELATIVE_VOTING_PERIOD_RE = re.compile(
    rf"주주\s*총회일\s*10\s*일\s*전\s*\(\s*"
    rf"{_DATE_TOKEN_RE.pattern}[^)]{{0,40}}\)\s*부터\s*"
    rf"주주\s*총회\s*전일\s*\(\s*"
    rf"{_DATE_TOKEN_RE.pattern}[^)]{{0,40}}\)\s*행사\s*가능"
    r"(?!\s*(?:하지\s*(?:않|아니)|아니|불가|여부|성|할지))"
    r"(?=\s*(?:[.。]|$))"
)
_ACTION_PRIORITY = {
    None: 0,
...
def _electronic_voting_mentions(
    clauses: list[str], evidence_base: dict[str, Any]
) -> list[dict[str, Any]]:
    mentions: list[dict[str, Any]] = []
    by_name: dict[str, int] = {}

    def manager_name(value: Any) -> str | None:
        name = _clean_text(value)
        if (
            _transaction_name(name) is None
            or re.fullmatch(
                r"(?:대표)?이사|감사|위원|담당자|임직원|직원|없음|미정|미확정",
                _compact(name),
            )
            is not None
        ):
    ...
    def append(
        *,
        ...
        if existing_index is not None:
            existing_services = mentions[existing_index]["attributes"]["services"]
            for service in services:
                if service not in existing_services:
                    existing_services.append(service)
            return
        by_name[name] = len(mentions)
        mentions.append(
            _mention(
                name=name,
    ...
    for clause in clauses:
        declaration = _VOTING_MANAGER_DECLARATION_RE.search(clause)
        declared_name = manager_name(declaration.group("name")) if declaration else None
    ...
    anaphoric_pattern = r"(?:이\s*(?:두\s*)?|두\s*)제도(?:의)?\s*관리\s*업무"
    delegation_statements: list[tuple[str, str, re.Match[str]]] = []
    for index, clause in enumerate(clauses):
        delegation = _VOTING_DELEGATION_RE.search(clause)
        if delegation is not None and _ELECTRONIC_VOTING_RE.search(
            clause[: delegation.start()]
        ) is not None:
            delegation_statements.append((clause, clause, delegation))

        for window_size in (2, 3):
            start = index - window_size + 1
            if delegation is None or start < 0:
                continue
            context_clauses = clauses[start:index]
            prefix = clause[: delegation.start()]
            anaphoric_statement = (
                re.search(anaphoric_pattern, prefix) is not None
                and any(
                    _ELECTRONIC_VOTING_RE.search(item) is not None
                    for item in context_clauses
                )
            )
            continued_statement = (
                bool(context_clauses)
                and _ELECTRONIC_VOTING_RE.search(context_clauses[-1]) is not None
                and re.search(anaphoric_pattern, context_clauses[-1]) is not None
                and _VOTING_DELEGATION_RE.search(context_clauses[-1]) is None
                and re.fullmatch(r"\s*(?:[-*]\s*)?", prefix) is not None
            )
            if anaphoric_statement or continued_statement:
                delegation_statements.append(
                    (" ".join([*context_clauses, clause]), clause, delegation)
                )

    seen_statements: set[tuple[str, str]] = set()
    for raw_text, clause, delegation in delegation_statements:
        delegated_name = manager_name(delegation.group("name"))
        if delegated_name is None:
            continue
        statement_key = (_compact(delegated_name), _compact(raw_text))
        if statement_key in seen_statements:
            continue
        seen_statements.add(statement_key)
        services = ["electronic_voting"]
        if _ELECTRONIC_PROXY_RE.search(raw_text) is not None:
            services.append("electronic_proxy")
        tail = clause[delegation.end() : delegation.end() + 24]
        if re.match(
            r"\s*(?:하지\s*(?:않|아니)|할\s*수\s*없|"
            r"(?:할\s*)?예정(?:이)?\s*없|철회|취소)",
            tail,
        ) is not None:
            continue
        append(
            ...
            delegation_status=(
                "planned"
                if _PLANNED_DELEGATION_RE.search(tail) is not None
                else "active"
            ),
            services=services,
        )
...
def _affirmative_voting_use_clauses(clauses: list[str]) -> list[str]:
    return [
        clause
        for clause in clauses
        if (
            _ELECTRONIC_VOTING_RE.search(clause) is not None
            or re.search(r"전자적\s*방법으로", clause) is not None
        )
        and _VOTING_AFFIRMATIVE_USE_RE.search(clause) is not None
        and _VOTING_NEGATIVE_USE_RE.search(clause) is None
    ]


def _has_explicit_voting_period(clauses: list[str]) -> bool:
    labelled_period = any(
        _VOTING_PERIOD_LABEL_RE.search(clause) is not None
        and len(_DATE_TOKEN_RE.findall(clause)) >= 2
        and re.search(r"(?:~|∼|〜|～|부터)", clause) is not None
        for clause in clauses
    )
    relative_period = _RELATIVE_VOTING_PERIOD_RE.search(" ".join(clauses)) is not None
    return labelled_period or relative_period


def _electronic_voting_system_provider_mentions(
    clauses: list[str], evidence_base: dict[str, Any]
) -> list[dict[str, Any]]:
    raw_text = " ".join(clauses)
    affirmative_use_clauses = _affirmative_voting_use_clauses(clauses)
    if not affirmative_use_clauses or not _has_explicit_voting_period(clauses):
        return []

    mentions: list[dict[str, Any]] = []
    services = ["electronic_voting"]
    if any(
        _ELECTRONIC_PROXY_RE.search(clause) is not None
        for clause in affirmative_use_clauses
    ):
        services.append("electronic_proxy")
    for match in _VOTING_SYSTEM_PROVIDER_RE.finditer(raw_text):
        name = _clean_text(
            match.group("named")
            or match.group("parenthesized")
            or match.group("terms")
        )
        provider_domain = _VOTING_PROVIDER_URLS.get(name)
        if provider_domain is None or provider_domain not in raw_text.casefold():
            continue
        provider_services = list(services)
        if (
            match.group("parenthesized") is not None
            and "electronic_proxy" not in provider_services
        ):
            provider_services.append("electronic_proxy")
        mentions.append(
            _mention(
                name=name,
                relationship_type="electronic_voting_system_provider_for",
                target_ref="@meeting",
                attributes={"usage_status": "active", "services": provider_services},
                evidence_base=evidence_base,
                raw_text=raw_text,
            )
        )
...
def extract_stakeholder_mentions(soup: Any) -> list[dict[str, Any]]:
    """Extract only explicitly named auditors and voting-service managers."""
    mentions: list[dict[str, Any]] = []
    seen: set[tuple[str, str, str, str]] = set()
    for cell, evidence_base in _reference_note_cells(soup):
        clauses = _clauses(cell)
        candidates = _external_auditor_mentions(clauses, evidence_base)
        voting_managers = _electronic_voting_mentions(clauses, evidence_base)
        manager_names = {str(mention["name"]) for mention in voting_managers}
        candidates.extend(voting_managers)
        candidates.extend(
            mention
            for mention in _electronic_voting_system_provider_mentions(
                clauses,
                evidence_base,
            )
            if str(mention["name"]) not in manager_names
        )
        for mention in candidates:
            ...
            key = (
                ...
                str(attributes.get("delegation_status", "")),
            )
            if key in seen:
                continue
            seen.add(key)
