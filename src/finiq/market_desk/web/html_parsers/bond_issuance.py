...
def parse_bond_issuance(html_text: str | bytes, *, file_path: str | Path) -> dict[str, Any]:
    ...
    if not rows:
        body_html = _fetch_selected_viewer_body(html_text, file_path=file_path)
...
def _fetch_selected_viewer_body(html_text: str | bytes, *, file_path: str | Path | None = None) -> bytes | None:
    """Attempt to find the content HTML for a given viewer HTML.
    
    If file_path is provided, it looks for a sibling 'viewer_html_contents' directory.
    Otherwise, it returns None as network fetching is disabled.
    """
    if not file_path:
        return None
