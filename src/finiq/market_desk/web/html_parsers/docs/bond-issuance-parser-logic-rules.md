...
| --- | --- |
| `mode` | `bond_issuance` |
| `title` fallback | 주입 제목이 없으면 첫 `SECTION-1` 문단. 둘 다 없으면 빈 문자열 |
| 판정용 제목 | `종류` 판정은 출력 `title` 기준. 본문 종류값은 쓰지 않음 |
| `<title>` 태그 | 제목 원천으로 사용하지 않음 |
...
| --- | --- |
| `mode` | `bond_issuance` |
| `title` | 주입 제목. 없으면 첫 `SECTION-1` 문단. 둘 다 없으면 빈 문자열 |
