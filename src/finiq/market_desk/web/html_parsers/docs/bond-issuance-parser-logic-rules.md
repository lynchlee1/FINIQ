...
| `기업명(행사대상)` | CB > EB > BW 순서의 전환/교환/인수권 행사 대상 주식 종류 행 | `parsed` | `source_not_found` | strong |
...
| `행사가액` | CB > EB > BW 순서의 전환/교환/행사가액 또는 가격 행 | `parsed` | `source_not_found` | strong |
...
| --- | --- |
| `종류` 판정 순서 | `BW`, `EB`, `CB` |
| `기업명(행사대상)` 정리 | 법인 형태와 주식 종류 표현을 제거하지 않고 원문 값을 반환 |
| `기업명(행사대상)` 탐색 순서 | `전환대상` -> `교환대상` -> `인수권행사대상` -> `전환에 따라` -> `교환에 따라` -> `인수권행사에 따라` -> `전환으로 발행할` -> `교환으로 발행할` -> `인수권행사로 발행할` |
| `기업명(행사대상)` 값 조건 | 기존 구조화 행과 긴 문단 추출을 모두 시도한 뒤에도 값이 없을 때만 fallback을 적용한다. fallback은 라벨 바로 다음 셀이 `종류`인 경우 그 다음 셀을 읽는다. 긴 문단 안에서는 `주식의 종류:` 또는 `유가증권:` 뒤의 120자 이하 값만 읽는다. |
| `행사가액` 탐색 순서 | `전환가액` -> `교환가액` -> `행사가액` -> `전환가격` -> `교환가격` -> `행사가격` |
| `행사가액` 값 조건 | 라벨 오른쪽 셀 중 숫자·공백·쉼표·소수점과 단위 `원`/`원/주`만 있는 값을 읽고, 설명 문장 속 숫자는 읽지 않는다. |
