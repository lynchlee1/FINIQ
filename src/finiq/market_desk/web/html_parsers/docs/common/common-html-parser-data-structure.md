...
- 상태가 `source_not_found`인 값은 `null`로 저장하고 반드시 strong 수준의 warning을 만든다. 정해진 하위 항목을 항상 나열하는 목록은 찾지 못한 하위 값을 `null`로 저장하고, 찾은 항목만 나열하는 목록은 전체 원천을 찾지 못했을 때 필드 자체를 `null`로 저장한다.
