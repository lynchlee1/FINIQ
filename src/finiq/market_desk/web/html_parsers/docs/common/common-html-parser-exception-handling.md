...
- `explicit_zero`이면 숫자 값은 0으로 저장한다.
- `source_not_found`이면 해당 값은 `null`로 저장한다.
- 목록 안의 일부 값만 찾지 못했으면 목록 형태는 유지하고 찾지 못한 값만 `null`로 저장한다.
- 목록 전체의 원천을 찾지 못했으면 빈 목록 대신 필드 자체를 `null`로 저장한다.
- `source_not_found`이면 반드시 `strong_warning`과 `parse_warnings`에 원천 누락 경고를 만든다.
- `field_parse_status_detail`의 세부 상태가 `source_not_found`인 경우도 같은 규칙을 적용한다.
  - 의도 : 명시적인 0과 원천 누락을 값만으로도 구분하고, 모든 원천 누락을 강한 경고로 확인하게 한다.
2. **상태 항목을 만드는 조건**
...
3. **파서별 추가 상태**
- 공시 종류에 따라 `not_applicable` 같은 상태를 추가로 사용할 수 있다.
