...
### 상태
1. **상태 기록 규칙**
- 기록할 상태가 있을 때만 `field_parse_status`를 만들고, 없다면 만들지 않는다.
- 기록할 세부 상태가 있을 때만 `field_parse_status_detail`을 만들고, 없다면 만들지 않는다.
...
- `explicit_zero`: 파서가 정해진 위치에서 대시 또는 0을 읽었다.
  - 이 경우 숫자 값은 0으로 저장한다.
- `source_not_found`: 파서가 정해진 값을 찾지 못했다. `strong_warning`과 `parse_warnings`에 원천 누락 경고를 만든다.
  - 원문에서 위치를 찾지 못한 경우, 값이 빈 경우, 숫자로 바꾸지 못한 경우 등이 포함될 수 있다.
  - 이 경우 해당 값은 `null`로 저장한다.
    - 주식 종류처럼 정해진 하위 항목을 항상 나열하는 목록은 목록 형태를 유지하고 찾지 못한 하위 값마다 `null`을 저장한다.
    - 발행목적이나 투자자처럼 찾은 항목만 나열하는 목록은 목록 전체의 원천을 찾지 못했으면 빈 목록 대신 필드 자체를 `null`로 저장한다.
- `not_applicable`: key는 있지만 value를 저장할 필요는 없다.
  - 파서는 해당 필드를 검색하거나 계산하지 않으며, 자동으로 `-`로 저장한다.
  - 원천 누락이 아니므로 경고를 만들지 않으며, 검증에서 제외한다.
  - value를 저장해야 하는데 값을 찾지 못한 경우를 `not_applicable`로 저장하는 동작은 **치명적인 실패**로 간주한다. `source_not_found`와 혼동하지 않도록 주의한다.
  - 의도 : 적용되지 않는 필드와 적용되지만 원천을 찾지 못한 필드를 명확하게 구분한다.
3. **상태 중앙 관리**
- 모든 하위 mode는 이 문서에 정의된 `parsed`, `explicit_zero`, `source_not_found`, `not_applicable`만 사용한다.
- 개별 parser 문서는 어떤 공시 종류의 어떤 필드에 상태를 적용하는지만 정한다
- 새로운 상태가 필요하면 먼저 이 공통 문서에 뜻과 저장·경고·검증 규칙을 추가하고 코드와 테스트를 같은 계약으로 변경한다.
- 공통 처리 과정은 parser가 기록한 상태를 다른 상태로 바꾸지 않는다.
  - 의도 : 상태 계약을 한 문서에서만 관리하고 개별 parser가 서로 다른 의미를 만들지 않게 한다.
...
- `skip_errors=True`인 경우 오류가 생기면 그 파일의 `record`와 `warning`을 기록하지 않고 오류를 `errors[]`에 기록한 다음 나머지 파일을 계속 처리한다.
  - 단, 확장자를 뺀 파일명이 같은 HTML 파일이 두 개 이상인 경우 · 정정공시 묶음인 `family`를 만드는 중 실패한 경우 · 최종 `payload`를 만들거나 저장하는 중 실패한 경우 등 치명적인 문제가 발생한 경우 `skip_errors`와 관계없이 전체 실행을 중단한다.
