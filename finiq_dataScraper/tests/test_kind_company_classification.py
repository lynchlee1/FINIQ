...
from finiq_dataScraper.data.facade import load_company_classification_file
from finiq_dataScraper.parse import companysummary_onclick, disclosure_rows
from finiq_dataScraper.storage.classification_store import company_classification_partial_path
from finiq_dataScraper.storage.classification_store import company_classification_shard_dir
import finiq_dataScraper.workflow.workflow as workflow_module
from finiq_dataScraper.workflow import (
...
def test_export_kind_company_classification_recurses_and_merges_same_company(
    ...
    index_payload = json.loads(output_path.read_text(encoding="utf-8"))
    payload = load_company_classification_file(output_path)

    assert index_payload["format"] == "company_classification_index_v2"
    assert company_classification_shard_dir(output_path).exists()
...
def test_export_kind_company_classification_deduplicates_overlapping_disclosures(
    ...
    index_payload = json.loads((tmp_path / "kind.company_classification.json").read_text(encoding="utf-8"))
