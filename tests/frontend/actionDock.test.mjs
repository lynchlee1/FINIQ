...
test("action dock notification panel can clear accumulated notifications", async () => {
  ...
  assert.match(source, /iconStyle\(visibleNotificationActive, openPanel === "notification", notificationTone\)/);
  assert.match(source, /notificationTone === "error"[\s\S]*?tv-down[\s\S]*?notificationTone === "warning"[\s\S]*?tv-warning[\s\S]*?notificationTone === "success"[\s\S]*?tv-up/);
  assert.match(source, /if \(!active \|\| tone === "neutral"\) return undefined/);
