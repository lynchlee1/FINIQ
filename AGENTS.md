...
- Before adding or changing UI labels, button names, page titles, status text, or feature names, consult `docs/design/terminology.md`. Reuse existing terms as much as possible; do not invent new button names or near-synonyms. If a new term is genuinely needed, add it there in the same change and keep UI/tests aligned with it.
